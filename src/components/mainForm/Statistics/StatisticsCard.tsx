...
export default function StatisticsCard({ title, movies }: { title: string; movies: string[] }) {
  const emotion = emotions.find((e) => title.includes(e.name));

  return (
    <div className="stats-card">
      <h3 className="stats-title">
        {emotion && <Image src={emotion.icon} alt={title} width={30} height={30} className="stats-emoji" />}
        {title} 콘텐츠 순위
      </h3>
